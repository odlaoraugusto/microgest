...
export default function ExamesListPage() {
  ...
  const [exames, setExames] = useState<Cultura[]>([]);
  ...
  async function handleLiberar(exame: Cultura) {
    try {
      await liberarExame(exame.id);
      carregar();
    } catch (err: unknown) {
  ...
  async function handleRemover(exame: Cultura) {
    const confirmar = window.confirm(
      `Remover o exame de "${exame.solicitacao?.material ?? ""}" do paciente "${
  ...
  return (
    <MainLayout titulo="Exames" subtitulo="Solicitação e resultado de cultura em um só lugar">
      ...
      <p style={{ marginTop: -8, marginBottom: 16, textAlign: "right" }}>
        <Link to="/solicitacoes/nova" style={{ fontSize: 12, color: "var(--mg-cinza-600)" }}>
          Agendar solicitação sem material coletado
        </Link>
      </p>
      ...
      <div className="mg-card">
        ...
        {/* ----- Visão: Todos os exames ----- */}
        {!erro && !carregando && visao === "todas" && exames.length === 0 && (
          <p style={{ color: "var(--mg-cinza-600)" }}>Nenhum exame cadastrado.</p>
        )}
                  ...
                  <tr key={c.id}>
                    ...
                    <td style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
                      <button
                        ...
                        onClick={() => navigate(`/exames/${c.id}/editar`)}
                      >
                        Continuar/Editar
                      </button>
        ...
        {!erro && !carregando && visao === "andamento" && parciais.length > 0 && (
          <>
            <p style={{ color: "var(--mg-cinza-600)", fontSize: 13, marginTop: -4, marginBottom: 16 }}>
              Exames que ainda não foram liberados, com a previsão de liberação e o que
            ...
            <p style={{ marginTop: 14, fontSize: 13, color: "var(--mg-cinza-600)" }}>
              {total} exame{total !== 1 ? "s" : ""} em andamento
            </p>
          </>
        )}
      </div>
